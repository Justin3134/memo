...
import { Check, X, Volume2, Upload, Phone } from "lucide-react";
...
const Landing = () => {
  ...
  const validatePatient = () => {
    ...
    if (!elderName.trim()) next.name = "Enter the patient's name.";
    if (!elderPhone.trim() || !/^\+?\d[\d\s().-]{6,}$/.test(elderPhone.trim())) {
      next.phone = "Enter a valid phone number including country code, e.g. +1 (555) 000-0000";
    }
    setErrors(next);
  ...
  const handleComplete = async () => {
    ...
    try {
      const patientId = await upsertPatientFromOnboarding({
        name: elderName.trim(),
        phone: elderPhone.trim(),
        callTime: "10:00",
        timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || "America/Chicago",
        familyMembers: familyMembers.filter(
          (m) => m.name.trim() || m.phone.trim() || m.relationship.trim()
        ),
      });
      ...
      try {
        ...
        // Non-fatal — patient is saved, call just couldn't be triggered right now
        setSubmitMessage(
          `Patient saved. Call could not be triggered: ${callErr instanceof Error ? callErr.message : "Unknown error"}`
        );
      }
      navigate("/dashboard");
    } catch (err) {
      setSubmitMessage(err instanceof Error ? err.message : "Failed to save setup.");
    } finally {
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-[13px] text-muted-foreground leading-relaxed max-w-sm">
              Set up a patient in under a minute. We'll call them right away.
            </p>
          </div>
            ...
            >
                ...
                <div className="space-y-3.5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 rounded-md border border-input bg-background text-foreground text-[13px] placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-foreground focus:border-foreground"
                    />
                  ...
                  <p className="text-[11px] text-muted-foreground leading-relaxed">
                    We'll call this number right after setup. Make sure they're available.
                  </p>
                </div>
              )}
              ...
              {currentStep === 1 && (
                <div className="space-y-4">
                  <p className="text-[12px] text-muted-foreground">Choose the voice {elderName || "the patient"} will hear on every call.</p>
              ...
              {currentStep === 2 && (
                <div className="space-y-3.5">
                  <p className="text-[12px] text-muted-foreground">Who should receive health updates? <span className="text-muted-foreground/60">(Optional — you can skip this)</span></p>
          ...
          <div className="flex gap-2.5 mt-6">
            ...
            <button
              onClick={() => {
                if (currentStep === 0 && !validatePatient()) return;
                if (currentStep < 2) {
              ...
              disabled={isSubmitting || (currentStep === 0 && !isStep0Valid)}
              className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-foreground text-background text-[13px] font-medium rounded-md hover:opacity-90 transition-opacity active:scale-[0.99] disabled:opacity-50"
            >
              {isSubmitting ? (
                "Calling…"
              ) : currentStep === 2 ? (
                <><Phone className="w-3.5 h-3.5" /> Finish Setup &amp; Call Now</>
              ) : (
                "Continue"
              )}
            </button>
          ...
          {submitMessage && (
            <p className="text-[11px] text-muted-foreground mt-3 leading-relaxed">{submitMessage}</p>
          )}
